...
for file in $(ls /opt/nwn2-stage 2>/dev/null || true); do
...
if [ -d /srv/nwnx4-user/plugins ] && [ "$(ls -A /srv/nwnx4-user/plugins/*.dll 2>/dev/null)" ]; then
...
if [ -d /opt/nwnx4/plugins ] && [ "$(ls -A /opt/nwnx4/plugins/*.dll 2>/dev/null)" ]; then
...
gosu nwnx4 bash <<-EOF
	...
	# Start Xvfb in background with better settings
	...
	wine reg import /opt/nwn2.reg 2>&1 | grep -v "fixme:" || true
